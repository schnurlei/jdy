...
<script  lang='ts'>
    ...
    @Component( {
        name: 'JdyTimestampFilterField',
        components: {
        }
    })
    export default class JdyTimestampFilterField extends Vue {
        ...
        get dateValue () {
            ...
            if (this.valueProperty) {
                timestamp =  (this.itemToEdit) ? this.itemToEdit[this.valueProperty] : null;
            } else {
        ...
        set dateValue (val: string | null ) {

            let dateVal = (val) ?  new Date(val) : null;
            if (this.valueProperty) {
                this.itemToEdit[this.valueProperty] = dateVal;
            } else if (this.itemToEdit  && this.primAttr) {
                this.itemToEdit[this.primAttr.getInternalName()] = dateVal;
            }
        }
