...
import {
    ...
    JdyClassInfo, JdyClassInfoQuery, JdyDataType, JdyFilterCreationException, JdyObjectListImpl,
    JdyRepository,
...
export function convertAppRepositoryToRepository (appRepository, callback) {

    appRepository.assocVals('Classes').done(function (appClassInfos: any[]) {
...
function buildAssocsForMetaRepo (metaRepo, anAppClassInfo): Promise<null> {
    ...
    var dfrd: Promise<null> = new Promise((resolve, reject) => {
        ...
        anAppClassInfo.assocVals('Associations').done(function (appAssocs) {
            ...
            let appAssoc;
            let metaMasterClass;
            let metaClass = metaRepo.getClassInfo(anAppClassInfo.InternalName);
            let metaMasterClassRef;
            let appAssocName;
            let metaAssoc;
...
function buildSubclassesForMetaRepo (metaRepo, anAppClassInfo) {
    ...
    let metaClass = metaRepo.getClassInfo(anAppClassInfo.InternalName);
    let appSuper = anAppClassInfo.Superclass;
    let metaSuper;
            ...
            for (i = 0; i < appAttributes.length; i++) {
                ...
                switch (appAttr.$typeInfo.internalName) {
                    ...
                    case 'AppObjectReference':
                        ...
                        break;
                    default:
                        reject(new Error('Invalid type: ' + appAttr.$typeInfo.internalName));
                }
...
export class FilterCreator {
    ...
    private rep: JdyRepository = createFilterRepository();
    private idCounter: number = 0;
    private curExpr: JdyTypedValueObject | null = null;

    public convertMetaFilter2AppFilter (metaQuery: JdyClassInfoQuery) {
        ...
        if (classInfo) {
            ...
            queryObj.setVal('repoName', metaQuery.getResultInfo().getRepoName());
            queryObj.setVal('className', metaQuery.getResultInfo().getInternalName());
    ...
    public createAppExpr (aMetaExpr) {
    ...
    public visitOrExpression (aOrExpr) {

        let classInfo: JdyClassInfo | null = this.rep.getClassInfo('AppOrExpr');
        if (classInfo) {
            let orExpr = new JdyTypedValueObject(classInfo, null, false);
            let subExprs = new JdyObjectListImpl(classInfo.getAssoc('orSubExpr'));
            let i;
    ...
    public visitAndExpression (aAndExpr: JdyAndExpression) {

        let classInfo: JdyClassInfo | null = this.rep.getClassInfo('AppAndExpr');
        if (classInfo) {
            let andExpr = new JdyTypedValueObject(classInfo, null, false);
            let subExprs = new JdyObjectListImpl(classInfo.getAssoc('andSubExpr'));
            let i;
    ...
    public createAppOperator (aMetaOper) {

        return aMetaOper.visitOperatorHandler(this);
    };

    public visitLessOperator (aOperator) {
    ...
    public visitGreatorOperator (aOperator) {
    ...
    public visitEqualOperator (aOperator) {
    ...
    return {
        getWorkflowStepsFor: function (aTypeInfo) {
            ...
            if (aTypeInfo.getInternalName() === 'AppRepository') {
                return [{ name: 'Close Repository', action: 'workflow.closeRepository' }, {
                    name: 'Open Repository',
                    action: 'workflow.openRepository'
                }];
            } else {
                return null;
            }
