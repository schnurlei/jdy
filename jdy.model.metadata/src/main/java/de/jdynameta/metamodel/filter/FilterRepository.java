...
public class FilterRepository extends JdyRepositoryModel
{
	...
	public static enum AppQueryName
	{
		className, repoName, expr, FilterId
	};

	public static enum AppOrExprName
	{
		orSubExpr
	};

	public static enum AppAndExprName
	{
		andSubExpr
	};
	...
	public FilterRepository() throws InvalidClassInfoException
	{
		...
		this.operatorExprModel.addReference(AppOperatorExprName.operator.name(), primitveOperatorModel).setDependent(true).setNotNull(true);
		this.operatorExprModel.addBooleanAttr(AppOperatorExprName.booleanVal.name());
		this.operatorExprModel.addDecimalAttr(AppOperatorExprName.decimalVal.name(), new BigDecimal("999999999.9999999"),new BigDecimal("999999999.9999999"),9);
		this.operatorExprModel.addFloatAttr(AppOperatorExprName.floatVal.name());
		this.operatorExprModel.addLongAttr(AppOperatorExprName.longVal.name(), -999999999, 999999999);
		this.operatorExprModel.addTextAttr(AppOperatorExprName.textVal.name(),1000);
		this.operatorExprModel.addTimestampAttr(AppOperatorExprName.timestampVal.name(),true,true);

		this.classInfoQueryModel = addClassInfo(TypeName.AppQuery.name()).setShortName("FQM");
		this.classInfoQueryModel.addLongAttr(AppQueryName.FilterId.name() ,0, 999999999).setIsKey(true).setNotNull(true).setGenerated(true);
		this.classInfoQueryModel.addTextAttr(AppQueryName.repoName.name() ,100).setNotNull(true);
		this.classInfoQueryModel.addTextAttr(AppQueryName.className.name() , 35).setNotNull(true);
		this.classInfoQueryModel.addReference(AppQueryName.expr.name(), filterExprModel);

		addAssociation(AppAndExprName.andSubExpr.name(), andExprModel, filterExprModel, false, false);
		addAssociation(AppOrExprName.orSubExpr.name(), orExprModel, filterExprModel, false, false);
